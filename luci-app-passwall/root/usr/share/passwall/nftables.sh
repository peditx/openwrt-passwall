...
add_firewall_rule() {
	...
	if [ -n "${WAN_IP}" ]; then
		...
		echolog "  - [$?]AppendWAN IParrivenftables：${WAN_IP}"
	fi
	unset WAN_IP
		...
		if [ -n "$UDP_NODE" -o "$TCP_UDP" = "1" ]; then
			_proxy_udp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo ip6 daddr ${2} $(REDIRECT $UDP_REDIR_PORT TPROXY6) comment \"local machine\""
