...
load_acl() {
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && [ "$CLIENT_PROXY" == 1 ] && {
		...
		[ "$UDP_NO_REDIR_PORTS" != "disable" ] && {
			nft "add $NFTABLE_NAME PSW_MANGLE ip protocol udp $(factor $UDP_NO_REDIR_PORTS "udp dport") counter return comment \"default\""
			nft "add $NFTABLE_NAME PSW_MANGLE_V6 counter meta l4proto udp $(factor $UDP_NO_REDIR_PORTS "udp dport") counter return comment \"default\""
			if [ "$UDP_NO_REDIR_PORTS" != "1:65535" ]; then
				echolog "     - ${msg}Not agent UDP port[${UDP_NO_REDIR_PORTS}]"
			else
		...
		[ -n "${TCP_PROXY_MODE}" -o -n "${UDP_PROXY_MODE}" ] && {
			...
			[ "$TCP_PROXY_DROP_PORTS" != "disable" ] && {
				...
				nft "add $NFTABLE_NAME $nft_prerouting_chain ip protocol tcp $(factor $TCP_PROXY_DROP_PORTS "tcp dport") ip daddr $FAKE_IP counter drop comment \"default\""
				[ "${USE_PROXY_LIST}" = "1" ] && nft "add $NFTABLE_NAME $nft_prerouting_chain ip protocol tcp $(factor $TCP_PROXY_DROP_PORTS "tcp dport") ip daddr @$NFTSET_BLACK counter drop comment \"default\""
				[ "${USE_GFW_LIST}" = "1" ] && nft "add $NFTABLE_NAME $nft_prerouting_chain ip protocol tcp $(factor $TCP_PROXY_DROP_PORTS "tcp dport") ip daddr @$NFTSET_GFW counter drop comment \"default\""
				[ "${CHN_LIST}" != "0" ] && nft "add $NFTABLE_NAME $nft_prerouting_chain ip protocol tcp $(factor $TCP_PROXY_DROP_PORTS "tcp dport") ip daddr @$NFTSET_CHN $(get_jump_ipt ${CHN_LIST} "counter drop") comment \"default\""
				[ "${USE_SHUNT_TCP}" = "1" ] && nft "add $NFTABLE_NAME $nft_prerouting_chain ip protocol tcp $(factor $TCP_PROXY_DROP_PORTS "tcp dport") ip daddr @$NFTSET_SHUNT counter drop comment \"default\""
				[ "${TCP_PROXY_MODE}" != "disable" ] && nft "add $NFTABLE_NAME $nft_prerouting_chain ip protocol tcp $(factor $TCP_PROXY_DROP_PORTS "tcp dport") counter drop comment \"default\""
				echolog "     - ${msg}Shield TCP port[${TCP_PROXY_DROP_PORTS}]"
			}
			...
			[ "$UDP_PROXY_DROP_PORTS" != "disable" ] && {
				...
				[ "${UDP_PROXY_MODE}" != "disable" ] && nft "add $NFTABLE_NAME PSW_MANGLE ip protocol udp $(factor $UDP_PROXY_DROP_PORTS "udp dport") counter drop comment \"default\""
				echolog "     - ${msg}Shield UDP port[${UDP_PROXY_DROP_PORTS}]"
			}
		}
...
add_firewall_rule() {
	...
	if [ -f $RULES_PATH/chnroute.nft ] && [ -s $RULES_PATH/chnroute.nft ] && [ $(awk 'END{print NR}' $RULES_PATH/chnroute.nft) -ge 8 ]; then
		#echolog "Use cache loadchnroute..."
	...
	if [ -f $RULES_PATH/chnroute6.nft ] && [ -s $RULES_PATH/chnroute6.nft ] && [ $(awk 'END{print NR}' $RULES_PATH/chnroute6.nft) -ge 8 ]; then
		#echolog "Use cache loadchnroute6..."
	...
	[ "$USE_DIRECT_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
				...
				echolog "  - [$?]Analyze and join[Direct list] GeoIP arrive NFTSET Finish"
			fi
		}
	}
	...
	[ "$USE_PROXY_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
				...
				echolog "  - [$?]Analyze and join[Proxy list] GeoIP arrive NFTSET Finish"
			fi
		}
	}
	...
	[ "$USE_BLOCK_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
				...
				echolog "  - [$?]Analyze and join[Shielding list] GeoIP arrive NFTSET Finish"
			fi
		}
	}
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
			...
			echolog "  - [$?]Analyze and join[Shunt node] GeoIP arrive NFTSET Finish"
		fi
	}
	...
	if [ -n "${WAN_IP}" ]; then
		...
		echolog "  - [$?]AddWAN IParrivenftables：${WAN_IP}"
	fi
	unset WAN_IP
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && {
		...
		if [ -n "$TCP_NODE" ]; then
			_proxy_tcp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					...
					if [ -z "${is_tproxy}" ]; then
						...
						nft insert rule $NFTABLE_NAME PSW_MANGLE ip protocol tcp iif lo tcp dport ${3} ip daddr ${2} $(REDIRECT $TCP_REDIR_PORT TPROXY4) comment \"Native machine\"
					fi
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo tcp dport ${3} ip6 daddr ${2} $(REDIRECT $TCP_REDIR_PORT TPROXY6) comment \"Native machine\""
			...
			[ -n "${LOCALHOST_TCP_PROXY_MODE}" ] && {
				...
				[ -n "${is_tproxy}" ] && nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol tcp iif lo $(REDIRECT $TCP_REDIR_PORT TPROXY4) comment \"Native machine\""
			}
			[ -n "${is_tproxy}" ] && nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol tcp iif lo counter return comment \"Native machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				[ -n "${LOCALHOST_TCP_PROXY_MODE}" ] && {
					...
					nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo $(REDIRECT $TCP_REDIR_PORT TPROXY) comment \"Native machine\""
				}
				nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo counter return comment \"Native machine\""
			}
		fi
		...
		if [ -n "$UDP_NODE" -o "$TCP_UDP" = "1" ]; then
			_proxy_udp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE ip protocol udp iif lo ip daddr ${2} $(REDIRECT $UDP_REDIR_PORT TPROXY4) comment \"Native machine\""
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo ip6 daddr ${2} $(REDIRECT $UDP_REDIR_PORT TPROXY6) comment \"Native machine\""
			...
			[ -n "${LOCALHOST_UDP_PROXY_MODE}" ] && {
				...
				nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol udp iif lo $(REDIRECT $UDP_REDIR_PORT TPROXY4) comment \"Native machine\""
			}
			nft "add rule $NFTABLE_NAME PSW_MANGLE ip protocol udp iif lo counter return comment \"Native machine\""
			...
			[ "$PROXY_IPV6" == "1" ] && {
				[ -n "${LOCALHOST_UDP_PROXY_MODE}" ] && {
					...
					nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto udp iif lo $(REDIRECT $UDP_REDIR_PORT TPROXY) comment \"Native machine\""
				}
				nft "add rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto udp iif lo counter return comment \"Native machine\""
			}
		fi
