...
filter_haproxy() {
	...
	echolog "  - [$?]Join the load balancing node tonftset[$NFTSET_VPS]直连完成"
}
...
filter_vpsip() {
	...
	echolog "  - [$?]加入所有IPv4节点到nftset[$NFTSET_VPS]Direct connection is completed"
...
add_firewall_rule() {
	...
	if [ -f $RULES_PATH/chnroute.nft ] && [ -s $RULES_PATH/chnroute.nft ] && [ $(awk 'END{print NR}' $RULES_PATH/chnroute.nft) -ge 8 ]; then
		#echolog "Loading with cachechnroute..."
	...
	if [ -f $RULES_PATH/chnroute6.nft ] && [ -s $RULES_PATH/chnroute6.nft ] && [ $(awk 'END{print NR}' $RULES_PATH/chnroute6.nft) -ge 8 ]; then
		#echolog "Loading with cachechnroute6..."
	...
	[ "$USE_DIRECT_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
				...
				echolog "  - [$?]Analyze and join[Direct link list] GeoIP arrive NFTSET Finish"
			fi
		}
	}
	...
	[ "$USE_PROXY_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
				...
				echolog "  - [$?]Analyze and join[Agent list] GeoIP arrive NFTSET Finish"
			fi
		}
	}
	...
	[ "$USE_BLOCK_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
				...
				echolog "  - [$?]Analyze and join[Block list] GeoIP arrive NFTSET Finish"
			fi
		}
	}
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		if [ -n "$GEOIP_CODE" ] && type geoview &> /dev/null; then
			...
			echolog "  - [$?]Analyze and join[Splitting node] GeoIP arrive NFTSET Finish"
		fi
	}
	...
	if [ -n "${WAN_IP}" ]; then
		...
		echolog "  - [$?]AdditionalWAN IParrivenftables：${WAN_IP}"
	fi
	unset WAN_IP
		...
		if [ -n "$UDP_NODE" -o "$TCP_UDP" = "1" ]; then
			_proxy_udp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					...
					nft "insert rule $NFTABLE_NAME PSW_MANGLE_V6 meta l4proto tcp iif lo ip6 daddr ${2} $(REDIRECT $UDP_REDIR_PORT TPROXY6) comment \"This machine\""
