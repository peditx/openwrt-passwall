	...
	TCP)
		...
		sing-box)
			...
			[ "${DNS_MODE}" != "sing-box" ] && [ "${DNS_MODE}" != "udp" ] && [ "$protocol" = "_shunt" ] && [ "$default_node" = "_direct" ] && {
				DNS_MODE="sing-box"
				v2ray_dns_mode="tcp"
				echolog "* currentTCPNode adoptionSing-BoxDistribution and the default node is directly connected，remoteDNSFilter mode will be used by defaultSing-Box(TCP)，Prevent loops！"
			}
		...
		xray)
			...
			[ "${DNS_MODE}" != "xray" ] && [ "${DNS_MODE}" != "udp" ] && [ "$protocol" = "_shunt" ] && [ "$default_node" = "_direct" ] && {
				DNS_MODE="xray"
				v2ray_dns_mode="tcp"
				echolog "* currentTCPNode adoptionXrayDistribution and the default node is directly connected，remoteDNSFilter mode will be used by defaultXray(TCP)，Prevent loops！"
			}
