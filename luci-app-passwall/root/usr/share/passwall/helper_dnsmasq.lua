...
function add_rule(var)
		...
		if USE_DIRECT_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/direct_host") then
				...
				if fwd_dns then
					...
					--Always use domesticDNS解析直连（白名单）List
		...
		if USE_PROXY_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/proxy_host") then
				...
				if fwd_dns then
					...
					--Always use remoteDNSAnalytical Agent（blacklist）List
	...
	if USE_CHINADNS_NG == "0" then
		log("  - PassWallMust depend onDnsmasq，If you configure the wrong configurationDNSprocess，Will result in a domain name(Direct connection/Proxy domain name)Splitting failure！！！")
	end
end
