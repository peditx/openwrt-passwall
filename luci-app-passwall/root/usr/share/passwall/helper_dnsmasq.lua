...
function add_rule(var)
	...
	if DEFAULT_PROXY_MODE == "proxy" and CHN_LIST == "0" and USE_GFW_LIST == "0" then
		--No Chinese list andGFWWhen the list is
		dnsmasq_default_dns = TUN_DNS
		only_global = 1
	end
	...
	if not fs.access(CACHE_DNS_PATH) then
		...
		if true then
			...
			if USE_CHINADNS_NG == "1" then
				...
				log(string.format("  - The domain name in the node list(vpslist)：%s", fwd_dns or "default"))
			end
		end
		...
		if USE_DIRECT_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/direct_host") then
				...
				if fwd_dns then
					...
					--Always use domesticDNSAnalysis（Whitelist）List
					...
					log(string.format("  - Domain name whitelin(whitelist)：%s", fwd_dns or "default"))
				end
			end
		end
		...
		if USE_PROXY_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/proxy_host") then
				...
				if fwd_dns then
					...
					log(string.format("  - Agent domain name table(blacklist)：%s", fwd_dns or "default"))
				end
			end
		end
		...
		if USE_GFW_LIST == "1" then
			if fs.access("/usr/share/passwall/rules/gfwlist") then
				...
				if fwd_dns then
					...
					log(string.format("  - Fire Wall Domain Table(gfwlist)：%s", fwd_dns or "default"))
				end
			end
		end
		...
		elseif only_global == 1 and NO_PROXY_IPV6 == "1" then
			...
			--Filter Agent domain name IPv6：Open up
			--Forbidden to analyze allIPv6Record
			list1["#"] = {
				dns = {},
				ipsets = {},
				address = "::"
	...
	if USE_CHINADNS_NG == "0" then
		log("  - PassWallMust depend onDnsmasq，If you configure the wrong yourselfDNSprocess，It will lead to a domain name(Direct connection/Proxy domain name)Diversion failure！！！")
	end
end
